...
def handle(argv):
  ...
  try:
    ...
    while(1):
        ...
        info2 = main_dnn_classify(frame)
        # MD_DETECT (motion detection)
        info3 = main_md_detect(ycfg, frame)

        ## Show frame stats...
        sys.stderr.write(' ### frame: ' + str(ystat['frame_id']) + ' ########################## ')
        sys.stderr.write(' * ' + 'dnn_classify.time'.ljust(25)     + ': ' + str(main_dnn_classify.time['time']) + ' (ms)\n')
        sys.stderr.write(' * ' + 'dnn_classify.hit'.ljust(25)      + ': ' + str(main_dnn_classify.time['hit'])+'\n' )
        sys.stderr.write(' * ' + 'dnn_classify.class_id'.ljust(25) + ': ' + info2['label2']+'\n')
        sys.stderr.write(' * ' + 'md_detect.time'.ljust(25)        + ': ' + str(main_md_detect.time['time'])    + ' (ms)\n')
    ...
    print("Errror "+ex.message)


  # Show global stats
  res  =' ### Global #################################### \n'
  res += ' * ' + 'Number of frames'.ljust(25) + ': ' + str(ystat['frame_id'])+'\n' 
  res += ' * ' + 'Number of hits'.ljust(25)   + ': ' + str(ystat['total_hits']) +'\n'
  res += ' * ' + 'Average time'.ljust(25)     + ': ' + str(ystat['total_time'] / ystat['frame_id']) + ' (ms)' +'\n'
